...
if(isset($_POST['save_venue'])){
    ...
    /* EVENT NAME */
    $ev = mysqli_fetch_assoc(mysqli_query($conn,"SELECT event_name FROM events WHERE event_id='$event_id'"));
    $event_name = $ev['event_name'];

    $folder = strtolower(str_replace(" ","_",$event_name));
...
<style>
/* SAME CSS (NO CHANGE) */
body{background:linear-gradient(135deg,#f5f3ff,#ede9fe);font-family:'Poppins',sans-serif;}
.main-content{margin-left:260px;padding:30px;}
h2{text-align:center;color:#5b21b6;margin-bottom:25px;}
form{background:white;padding:30px;border-radius:20px;box-shadow:0 15px 40px rgba(91,33,182,0.15);max-width:650px;margin:0 auto 40px auto;}
input,select{width:100%;padding:12px;margin-top:10px;border-radius:12px;border:1px solid #ddd;}
button{margin-top:20px;padding:14px;border:none;border-radius:30px;background:#7c3aed;color:white;cursor:pointer;width:100%;}
table{width:100%;border-collapse:collapse;background:white;border-radius:20px;overflow:hidden;box-shadow:0 15px 40px rgba(91,33,182,0.15);}
th{background:#7c3aed;color:white;padding:15px;}
td{padding:15px;border-bottom:1px solid #eee;}
img{width:85px;height:65px;object-fit:cover;border-radius:12px;}
.btn{padding:6px 14px;border-radius:20px;color:white;text-decoration:none;font-size:13px;}
...
<!-- ✅ EVENT SELECT -->
<select name="event_id" id="event_select" required onchange="loadPackages()">
...
fetch("get_packages.php?event_id="+eventId)
.then(res=>res.text())
.then(data=>{
document.getElementById("package_select").innerHTML=data;
});
}
</script>
